<template>
  <header class="bg-white h-20 flex items-center">
    <div class="container mx-auto px-5 flex justify-between items-center text-customBlack">
      ...
      <nav class="hidden md:block">
        <ul class="flex gap-7 items-center">
          <li v-for="(modal, id) in navbarModals" @click="openModal(modal.name)" :key="'navbarModals' + id" :class="{
...
const openModal = (name) => {
  ...
  if (name === 'Вакансия') isOpenVacanciesModal.value = true
  if (name === 'Войти') isOpenLoginModal.value = true
