...
<template>
  <header class="bg-white h-20 flex items-center">
    <div class="container mx-auto flex justify-between items-center px-5">
      ...
      <div class="flex gap-2 items-center relative cursor-pointer" ref="avatarTrigger" @click="isOpenAvatar = !isOpenAvatar">
        ...
        @click.stop 
        v-show="isOpenAvatar"
        class="absolute w-32 h-32 -bottom-[140px] right-0 bg-white shadow-lg flex flex-col gap-2 px-3 py-1.5 ">
       <router-link v-for="(link,id) in avatarLinks" class="text-customBlack font-rubik"
       :key="'avatar-link-id' + id" :to="link.to">
        <span v-if="link.role">
            {{ link.title }}
        </span>
        </router-link>
